...
describe("useUrlTab", () => {
  it("returns the default tab when the URL has no param", () => {
    const { result } = renderHook(() => useUrlTab({ tabs: TABS, defaultTab: "overview" }), {
  ...
  it("keeps the param when picking the default tab with clearOnDefault: false", async () => {
    const onUrlUpdate = vi.fn<(event: UrlUpdateEvent) => void>();
    const { result } = renderHook(() => useUrlTab({ tabs: TABS, defaultTab: "overview", clearOnDefault: false }), {
      wrapper: withNuqsTestingAdapter({ searchParams: "?tab=logs", onUrlUpdate }),
    });
    await act(async () => {
      result.current[1]("overview");
  ...
  it("uses a custom param name when given", () => {
    const { result } = renderHook(() => useUrlTab({ tabs: TABS, defaultTab: "overview", paramName: "view" }), {
